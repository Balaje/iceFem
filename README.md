...
| ![Solution](./Images/orien1.png) | ![Meshes](./Images/orien2.png) |
| ------------------------------ | ------------------------------ |

**NOTE: The order of clicking does not matter, as the code transforms the coordinates to a predefined coordinate setting as shown in the pictures above. The shelf front is placed at `x=0` and the open-ocean region is placed at `z=0, x<0`.**
...
```
in `include/spline.idp`.  

The meshes are placed at the coordinates system where with the front of the ice-shelf is at `x=0` and the open ocean being at `z=0`. Figure below shows the profiles of the ice-shelf and the cavity along with a snapshot of the shelf-front.

|![Profile](./Images/profileMesh.png)|
| -- |

The full program to solve the BEDMAP2 problem including translating the ice-shelf to `x=0` at the open-ocean to `z=0, x<0`; is available in `bedMapProfile.m`. The following FreeFem++ command can be used to run the program from the terminal once the meshes are generated once.

```
FreeFem++ -nw -ne solveBEDMAP2.edp -isMesh 0 -Tr 200 -Ti 0 -nborders 4
```
where the argument
 - `-isMesh` tells the FreeFem++ code to generate the mesh from the cubic spline data. Could be set to `0` if the mesh is available in the folder.
 - The argument `-nborders` tells the FreeFem++ code to restrict the border to within the shelf region (4 boundaries for the cavity region). Any other value would generate the solution including the open-ocean region (See Example 2). 

 The figure below shows the deformation of the ice-shelf and the velocity potential (zoomed in for better visualization) together with a sample output window.

| ![Brunt](./Images/bruntSol22.png) | ![Output](./Images/cmdWindow.png) |
| --- | --- |

The absolute value of reflection coefficient is equal to 1 indicating conservation of energy:

| Reflection Coefficient, R | abs(R)  |
| ---------------------------------- | ------------------------------ |
| (0.7720865633805307,0.6355173787137782)| 0.999999999999995 |
...
|--| -- |
| R = (0.8220502489967969,-0.5735989034071053) | R=(-0.5429927446321565,-0.8405980481250369) |
| abs(R) = 1.002388304932539 | abs(R)= 1.000722838369738|
