...
The full program to solve the BEDMAP2 problem after translating the ice-shelf to `x=0` at the open-ocean to `z=0` is available in `bedMapProfile.m`. The command

```
FreeFem++ -nw -ne solveBEDMAP2.edp -isMesh 1 -Tr 200 -Ti 0 -nborders 4
```
where the argument `-isMesh` tells the FreeFem++ code to generate the mesh from the cubic spline data. The argument `-nborders` tells the FreeFem++ code to restrict the border to within the shelf region (4 boundaries for the cavity region). The figure below shows the deformation of the ice-shelf and the velocity potential (scaled down for better visualization).

| ![Brunt](./Images/bruntSol22.png) |
| --- |
...
| Reflection Coefficient, R | abs(R)  |
| ---------------------------------- | ------------------------------ |
| (-0.2343914034320831,-0.972142309539647) | 0.9999999999999701 |
